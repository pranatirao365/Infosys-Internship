import React, { useState } from 'react';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Legend } from 'recharts';

function DistributionAnalysis({ data, selectedPollutants }) {
  const [isFullscreen, setIsFullscreen] = useState(false);
  
  if (!data || data.length === 0) {
    return (
      <>
        <div className="card-header">
          <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
          </svg>
          Distribution Analysis
        </div>
        <div className="empty-state">
          <h3>No Data Available</h3>
          <p>Select filters and click "Apply Filters" to view distribution</p>
        </div>
      </>
    );
  }

  const getPM25 = (item) => {
    if (item.PM2 && typeof item.PM2 === 'object' && item.PM2['5'] !== undefined) {
      return item.PM2['5'];
    }
    if (item['PM2.5'] !== undefined) {
      return item['PM2.5'];
    }
    return null;
  };

  const getPollutantValue = (item, pollutant) => {
    if (pollutant === 'PM2.5') return getPM25(item);
    return item[pollutant] || null;
  };

  const activePollutants = selectedPollutants && selectedPollutants.length > 0 ? selectedPollutants : ['PM2.5'];

  const bins = [
    { range: '0-20', min: 0, max: 20 },
    { range: '20-40', min: 20, max: 40 },
    { range: '40-60', min: 40, max: 60 },
    { range: '60-80', min: 60, max: 80 },
    { range: '80-100', min: 80, max: 100 },
    { range: '100+', min: 100, max: Infinity }
  ];

  const distributionData = bins.map(bin => {
    const dataPoint = { range: bin.range };
    activePollutants.forEach(pollutant => {
      const values = data.map(d => getPollutantValue(d, pollutant)).filter(v => v != null && !isNaN(v) && v >= 0);
      const count = values.filter(v => v >= bin.min && v < bin.max).length;
      dataPoint[pollutant] = count;
    });
    return dataPoint;
  });

  const colors = {
    'PM2.5': '#4caf50', 'PM10': '#2196f3', 'NO': '#ff9800', 'NO2': '#f44336',
    'NOx': '#9c27b0', 'NH3': '#00bcd4', 'CO': '#795548', 'SO2': '#e91e63',
    'O3': '#3f51b5', 'Benzene': '#ff5722', 'Toluene': '#607d8b', 'AQI': '#8bc34a'
  };

  return (
    <>
      <div className={`${isFullscreen ? 'fullscreen-card' : ''}`}>
      <div className="card-header">
        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
        </svg>
        Distribution Analysis
        <button 
          onClick={() => setIsFullscreen(!isFullscreen)}
          style={{
            marginLeft: 'auto',
            padding: '6px 12px',
            background: '#f5f5f5',
            color: '#666',
            border: '1px solid #ddd',
            borderRadius: '4px',
            cursor: 'pointer',
            fontWeight: '500',
            fontSize: '0.8rem',
            display: 'flex',
            alignItems: 'center',
            gap: '4px',
            transition: 'all 0.2s ease'
          }}
          onMouseOver={(e) => {
            e.currentTarget.style.background = '#e8e8e8';
            e.currentTarget.style.borderColor = '#ccc';
          }}
          onMouseOut={(e) => {
            e.currentTarget.style.background = '#f5f5f5';
            e.currentTarget.style.borderColor = '#ddd';
          }}
        >
          <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" style={{width: '14px', height: '14px'}}>
            {isFullscreen ? (
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
            ) : (
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4" />
            )}
          </svg>
          {isFullscreen ? 'Exit' : 'Fullscreen'}
        </button>
      </div>
      <ResponsiveContainer width="100%" height={isFullscreen ? 650 : 360}>
        <BarChart data={distributionData} margin={{ top: 25, right: 20, bottom: 60, left: 60 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#e0e0e0" strokeWidth={0.5} />
          <XAxis 
            dataKey="range" 
            tick={{fontSize: 8, fill: '#333', fontWeight: 500}} 
            stroke="#999"
            strokeWidth={0.5}
            height={45}
            label={{ 
              value: 'Concentration Range (µg/m³)', 
              position: 'insideBottom', 
              offset: -10,
              style: { fontSize: 11, fill: '#333', fontWeight: 600 }
            }}
          />
          <YAxis 
            tick={{fontSize: 8, fill: '#333', fontWeight: 500}} 
            stroke="#999"
            strokeWidth={0.5}
            width={50}
            label={{ 
              value: 'Frequency Count', 
              angle: -90, 
              position: 'insideLeft',
              offset: 10,
              style: { fontSize: 11, fill: '#333', fontWeight: 600, textAnchor: 'middle' }
            }}
          />
          <Tooltip 
            contentStyle={{ 
              background: 'white', 
              border: '1px solid #ddd', 
              borderRadius: '4px', 
              padding: '6px 10px',
              boxShadow: '0 2px 8px rgba(0,0,0,0.1)'
            }}
            labelStyle={{
              color: '#000',
              fontWeight: 600,
              marginBottom: '3px',
              fontSize: '10px'
            }}
            itemStyle={{
              color: '#333',
              fontSize: '9px'
            }}
            cursor={{ fill: 'rgba(0, 0, 0, 0.05)' }}
          />
          <Legend 
            wrapperStyle={{fontSize: '8px', paddingTop: '0px', marginTop: '-20px', color: '#000'}} 
            iconType="rect"
            iconSize={8}
            verticalAlign="top"
            height={30}
          />
          {activePollutants.map((pollutant, index) => (
            <Bar 
              key={pollutant} 
              dataKey={pollutant} 
              fill={colors[pollutant] || '#888'}
              fillOpacity={0.7}
              name={pollutant} 
              radius={[4, 4, 0, 0]}
              animationDuration={800}
              animationEasing="ease-out"
            />
          ))}
        </BarChart>
      </ResponsiveContainer>
      </div>
      {isFullscreen && (
        <div 
          onClick={() => setIsFullscreen(false)}
          style={{
            position: 'fixed',
            top: 0,
            left: 0,
            right: 0,
            bottom: 0,
            background: 'rgba(0,0,0,0.7)',
            zIndex: 998
          }}
        />
      )}
    </>
  );
}

export default DistributionAnalysis;
